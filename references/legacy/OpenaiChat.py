...
class OpenaiChat(AsyncAuthedProvider, ProviderModelMixin):
    ...
    async def get_generated_image(cls, session: StreamSession, auth_result: AuthResult, element: Union[dict, str],
        ...
        if is_sediment:
            url = f"{cls.url}/backend-api/files/download/{element}?conversation_id={conversation_id}&inline=false"
        else:
    ...
    async def iter_messages_line(cls, session: StreamSession, auth_result: AuthResult, line: bytes,
        ...
        if "v" in line:
            ...
            if isinstance(v, str) and fields.recipient == "all":
                ...
                # Handle "add" delta operations with image_asset_pointer
                msg = v.get("message", {})
                ...
                for part in parts:
                    if isinstance(part, dict) and part.get("content_type") == "image_asset_pointer":
                        ...
                        if asset:
                            ...
                            generated_images = fields.generated_images = await cls.get_generated_image(
                                session, auth_result, asset,
                                fields.prompt, fields.conversation_id, status)
                            if generated_images is not None:
                                yield generated_images
                # Update conversation_id and recipient
                if v.get("conversation_id"):
                    fields.conversation_id = v["conversation_id"]
                if msg.get("recipient"):
                    fields.recipient = msg["recipient"]
            ...
            try:
                ...
                if has_nodriver:
                    if cls.request_config.access_token is None:
                        yield RequestLogin(cls.label, os.environ.get("CHATGPT_LOGIN_URL", ""))
